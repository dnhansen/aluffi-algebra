...
\begin{solution}
    ...
    We also have the following corollary:\footnote{This result allows us to solve congruences like $x^k \equiv b \bmod n$ if we know $\phi(n)$, and $k$ and $\phi(n)$ are coprime. Bezout's identity furnishes $u,v \in \ints$ such that $ku - \phi(n)v = 1$, which implies that
    %
    \begin{equation*}
        b^u
            \equiv x^{ku}
    ...
    \end{equation*}
    %
    Notice that $u$ is in fact a multiplicative inverse for $k$ mod $\phi(n)$; of course any such inverse will do.
