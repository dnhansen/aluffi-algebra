...
\begin{remark}
    We elaborate on the claim that Proposition~2.7 is a \enquote{ring analogue} of Cayley's theorem. The latter says that any group $G$ acts faithfully on some set, namely itself by left multiplication. In other words, there is an injective group homomorphism
    %
    \begin{equation*}
...
\end{exerciseframed}
%
If $H$ is only normal in $K$, then it is not necessarily normal in $G$. If normality is transitive among subgroups of $G$, then $G$ is called a \emph{T-group}. The smallest group that is not a $T$ group is the dihedral group $D_8$ of order $8$: Let $K$ be the Klein four group (e.g. generated by rotation by $180$ degrees along with two reflections), and let $H$ be generated by a reflection.

Also note that if $K$ is also characteristic in $G$, then $H$ is characteristic in $G$ (by an analogous argument to (b)).
...
\begin{exerciseframed}[4.3.3]
    Prove that every finite group has a composition series. Prove that $\ints$ does not have a composition series.
\end{exerciseframed}

\begin{solution}
    Let $G$ be a finite group. If $G$ is simple then there is nothing to prove, and otherwise let $N$ be a normal subgroup of $G$ of maximal order. Then $N$ has a composition series by induction, and this extends to a composition series for $G$ since $G/N$ is simple (by the third isomorphism theorem).

    A composition series for $\ints$ must have the form
    %
    \begin{equation*}
        \ints
            \supsetneq n_1 \ints
    ...
    \end{equation*}
    %
    where $n_k \neq 1$. But $n_k \ints / \{0\} \cong n_k \ints$ is not simple.
\end{solution}
